...
class MagazineModule:
    ...
    def render_magazine(self, period='7일', start_date=None, end_date=None, selected_magazines=None):
        ...
        try:
            ...
            try:
                # 키워드 연관어 분석 실행
                from core.analyzer import Analyzer  # analyzer.py에서 Analyzer 임포트
                
                # 공통 키워드 연관어 분석
                if common_keywords:
                    self.logger.info(f"공통 키워드 연관어 분석 시작: {common_keywords}")
                    keyword_analysis = Analyzer.get_common_keyword_association(data, common_keywords)
                    
                    # 파이차트 생성
                    keyword_charts = Analyzer.generate_keyword_pie_charts(keyword_analysis)
                    self.logger.info(f"키워드 파이차트 생성 완료: {list(keyword_charts.keys())}")
                else:
                    self.logger.warning("공통 키워드가 없어 연관어 분석을 건너뜁니다.")
                    keyword_analysis = {}
                    keyword_charts = {}
                
                # 기존 다른 시각화 생성 (워드클라우드, 네트워크 등)
                ...
                # fig_trend는 제거하고 대신 keyword_charts 사용
                ...
                keyword_analysis = {}
                keyword_charts = {}
                fig_wordcloud = fig_network = fig_category = None

            # 템플릿 렌더링 부분 (return render_template 부분 수정)
            return render_template('magazine.html',
                                ...
                                keyword_charts=keyword_charts,  # 새로 추가된 키워드 파이차트
                                keyword_analysis=keyword_analysis,  # 키워드 분석 결과 추가
                                fig_wordcloud=fig_wordcloud,
                                fig_network=fig_network,
                                fig_category=fig_category)
