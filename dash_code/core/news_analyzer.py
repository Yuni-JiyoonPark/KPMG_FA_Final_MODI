...
# Stopwords 정의
STOPWORDS = set([
    '것', '수', '등', '더', '위해', '또한', '있는', '최대', '확보', '결과', '성공', '하는', '에서', '으로',
    '그리고', '이번', '한편', '있다', '했다', '대한', '가장', '이런',
    '한다', '한다면', '바', '때', '다양한', '통해', '기자', '최근', '제공', '선정', '기회'
...
def prepare_text_data(self, remove_stopwords=True):
        """텍스트 데이터 준비
        ...
        # 토큰 리스트 처리
        self.data['processed_tokens'] = self.data['token_list'].apply(process_tokens)
        
        # 토큰을 문자열로 결합한 컬럼 추가
        self.data['token_text'] = self.data['processed_tokens'].apply(
...
class NewsAnalyzer:
    ...
    # news_analyzer.py의 generate_keyword_trend_chart 함수 수정
        ...
        try:
            ...
            fig.update_layout(
                ...
                xaxis=dict(
                    ...
                    automargin=True  # X축 레이블이 잘리지 않도록 자동 여백 설정
                ),
                yaxis=dict(
                    title='언급 빈도',
                    gridcolor='#eee',
                ...
                legend=dict(
                    ...
                    xanchor="right",
                    x=1,
                    font=dict(size=12)  # 범례 폰트 크기 축소
                ),
                template='plotly_white',
                # 고정 높이 제거
                autosize=True,  # 자동 크기 조정 활성화
                margin=dict(l=30, r=20, t=50, b=50)  # 여백 축소
            )
            
            # 반응형 설정
            config = {
                'responsive': True,
                'displayModeBar': False  # 모드 바 숨김
            }
    ...
    # news_analyzer.py의 generate_network_graph 함수 수정
    def generate_network_graph(self, association_results=None, max_nodes=15):
        """단어 네트워크 그래프 생성"""
        ...
        for node in G_vis.nodes():
            ...
            # 노드 크기 (단어 빈도에 비례)
            size = np.sqrt(word_counts[node]) * 1.8  # 크기 조정 (기존 2.0 -> 1.8)
        ...
        node_trace = go.Scatter(
            ...
            textfont=dict(size=10),  # 텍스트 크기 축소
            hoverinfo='text',
            hovertext=node_text,
            marker=dict(
                showscale=True,
                ...
                size=node_size,
                colorbar=dict(
                    thickness=10,  # 컬러바 두께 축소
                    title=dict(
                        text='단어 빈도',
                        side='right',
                        font=dict(size=10)  # 컬러바 제목 폰트 크기 축소
                    ),
                    xanchor='left',
                    len=0.5  # 컬러바 길이 축소
                ),
                color=[word_counts[node] for node in G_vis.nodes()],
                line=dict(width=1)  # 노드 테두리 두께 축소
            )
        )
        ...
        fig = go.Figure(data=[edge_trace, node_trace],
                    layout=go.Layout(
                        ...
                        showlegend=False,
                        hovermode='closest',
                        margin=dict(b=10, l=5, r=5, t=40),  # 하단 여백 축소
    ...
    def generate_frequency_chart(self, freq_results=None, top_n=30):
        ...
        fig.update_layout(
            width=600,  # 기본 너비 설정
            height=400,  # 기본 높이 설정
            autosize=True,  # 컨테이너에 맞게 자동 조정
            margin=dict(l=20, r=20, t=30, b=30)  # 여백 최소화
        )
